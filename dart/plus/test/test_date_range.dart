...
// custom <main>
  ...
  group('test_date_rage.dart', () {
    ...
    group('IntervalDateRange', () {
      var idrRanges = intervalDateRange(date(2001,1,31),
          date(2004,3,1), Frequency.MONTHLY).dateRanges;
      ...
      expect(idrRanges[36], dateRange(date(2004,1,31), date(2004,2,29))); // leap
      expect(idrRanges[37], dateRange(date(2004,2,29), date(2004,3,1)));

      test('contains', () {
        expect(
          dateRange(date(2001, 1, 1), date(2010,1,1))
          .contains(date(2001,1,2)),
          true);

        expect(
          dateRange(date(2001, 1, 1), date(2010,1,1))
          .contains(date(2001,1,1)),
          true);

        expect(
          dateRange(date(2001, 1, 1), date(2010,1,1))
          .contains(date(2010,1,2)),
          false);

        expect(
          dateRange(date(2001, 1, 1), date(2010,1,1))
          .contains(date(2010,1,1)),
          false);
      ...
      test('containsYear', () {
        expect(
          dateRange(date(2000, 1, 1), date(2010,1,1)).containsYear(2001), true);
        expect(
          dateRange(date(2001, 1, 1), date(2010,1,1)).containsYear(2001), true);
        expect(
          dateRange(date(2002, 1, 1), date(2010,1,1)).containsYear(2001), false);
        expect(
          dateRange(date(2000, 1, 1), date(2001,1,1)).containsYear(2001), false);
      });
      ...
      test('subranges work monthly', () {
        ...
        var multipleIntervals =
          idrRanges.fold(1.0, (prev, range) =>
              prev * rc.scaleFromTo(range.start, range.end));
      ...
      test('fiscalRange should return SOY to SOY', () {
        expect(fiscalRange(1944), dateRange(date(1944,1,1), date(1945,1,1)));
      });
