...
class RateCurve {
  ...
  RateCurve copy() =>
    new RateCurve(
      new List
      .from(_curveData.map((dv) => dv.copy())));
  ...
  operator *(num scalar) {
    ...
    for(int i=0; i<_curveData.length; i++) {
      guts.add(dv(_curveData[i].date,
              _curveData[i].value * scalar));
    }
  ...
  static RateCurve merge(final RateCurve c1, final RateCurve c2) {
    ...
    int c2Len = c2.length;
    int c1i = 0, c2i = 0;
    double c1Last = 0.0, c2Last = 0.0, lastMerged = 0.0;

    while(c1i < c1Len || c2i < c2Len) {
      bool useLeft = ((c2i == c2Len) ||
          ((c1i < c1Len) &&
              (c1._curveData[c1i].isBefore(c2._curveData[c2i]))));
  ...
  static _append(List<DateValue> data, DateValue dv) {
    ...
    if(data.length == 1 && data[0].value == 0.0)
      data.clear();
  }
