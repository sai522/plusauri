...
class TradeJournal {
  ...
  int get hashCode => hash2(const ListEquality<Trade>().hash(_trades), _dateRange);

  copy() => new TradeJournal._default()
    .._trades = _trades == null? null :
    (new List.from(_trades.map((e) =>
      e == null? null : e.copy())))
  ...
  Iterable<Trade> after(Date date) =>
    _trades.where((t) => t.date >= date);
  ...
  static TradeJournal fromJson(Object json) {
    ...
    assert(json is Map);
    return new TradeJournal._default()
  ...
  void _fromJsonMapImpl(Map jsonMap) {
    ...
    _trades = ebisu_utils
      .constructListFromJsonData(jsonMap["trades"],
...
class TradeAccountCollection {
  ...
  static TradeAccountCollection fromJson(Object json) {
    ...
    assert(json is Map);
    return new TradeAccountCollection._default()
      .._fromJsonMapImpl(json);
  }

  void _fromJsonMapImpl(Map jsonMap) {
    // tradeJournalMap is Map<String,TradeJournal>
    tradeJournalMap = ebisu_utils
...
''';
  ...
  CostBasisEntry._copy(CostBasisEntry other) :
    ...
    offsettingBuys = other.offsettingBuys == null? null :
      (new List.from(other.offsettingBuys.map((e) =>
        e == null? null : e.copy()))),
    cost = other.cost,
    shortTermGain = other.shortTermGain,
    longTermGain = other.longTermGain,
    lotCloseMethod = other.lotCloseMethod == null? null : other.lotCloseMethod.copy();
...
abstract class LotCloser {
  ...
  closeLots(TradeTaxAssessor tradeTaxAssessor, DateRange onRange) {
    ...
    while(openLots.length > 0) {
      ...
      if(Logger.root.level <= Level.FINER)
        _logger.finer('OpenLots => $openLots => $nextSellIndex');
      ...
      if(nextSellIndex > 0) {
        ...
        if(Logger.root.level <= Level.FINER)
          _logger.finer('MR => $matchResult => ${matchResult.createCostBasisEntry()}');

        tradeTaxAssessor
          ._addCostBasisEntry(matchResult.createCostBasisEntry());
    ...
    if(Logger.root.level <= Level.FINER)
      _logger.finer('Remainging lots ${openLots.length}');
  }
...
class MatchResult {
  ...
  factory MatchResult.fromBuys(Iterable<Trade> allBuys, Trade sell,
    ...
    for(Trade buy in allBuys) {
      ...
      if(toBeSold > buyQty) {
        ...
        matchedBuys.add(
          (new TradeBuilder.copyFrom(buy)
              ..quantity = toBeSold).buildInstance());
  ...
  CostBasisEntry createCostBasisEntry() {
    ...
    double cost = 0.0, shortTermGain = 0.0, longTermGain = 0.0;
    ...
    assert((longTermGain + shortTermGain - (sell.marketValue - cost)).abs() < 0.0001);
    return new CostBasisEntry(sell, matchedBuys, cost, shortTermGain,
        longTermGain, lotCloseMethod);
  }
