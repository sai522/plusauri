...
class Capitalization implements Comparable<Capitalization> {
  ...
  static get values => [LARGE_CAP, MID_CAP, SMALL_CAP];
...
class InvestmentStyle implements Comparable<InvestmentStyle> {
  ...
  static get values => [VALUE, BLEND, GROWTH];
...
class LotCloseMethod implements Comparable<LotCloseMethod> {
  ...
  static get values => [FIFO, LIFO, HIFO, LOW_COST, AVERAGE_COST];
...
class PRResults {
  ...
  static PRResults fromJson(Object json) {
    ...
    assert(json is Map);
    return new PRResults._default().._fromJsonMapImpl(json);
  }
...
PRResults pRResults([Map<String, TimeSeries> prices]) => new PRResults(prices);
...
PRRequest pRRequest([DateRange dateRange, Set<String> symbols]) =>
    new PRRequest(dateRange, symbols);
