...
class _IncomeSpecs {
  get partTimeJob => new IncomeSpec(IncomeType.LABOR_INCOME, new FlowSpec(
      "Grocery store job", "source", cFlowSequenceSpec()
  ...
  get fullTimeJob => new IncomeSpec(IncomeType.LABOR_INCOME, new FlowSpec(
      "Pharmacy store job", "source", cFlowSequenceSpec()
    ..dateRange = dateRange(date(2010, 1, 1), date(2025, 1, 1))
    ..growth = _curves.fullTimeLaborGrowth
    ..paymentFrequency = PaymentFrequency.ANNUAL
    ..initialValue = dv(date(2013, 1, 1), 80000.0)));

  get pensionIncome => new IncomeSpec(IncomeType.PENSION_INCOME, new FlowSpec(
      "Small pension", "source", cFlowSequenceSpec()
...
class _ExpenseSpecs {
  get middleClassLifeGross => new ExpenseSpec(ExpenseType.LIVING_EXPENSE,
  ...
  get economicChildCollege => new ExpenseSpec(ExpenseType.COLLEGE_EXPENSE,
  ...
  get smartChildCollege => new ExpenseSpec(ExpenseType.COLLEGE_EXPENSE,
  ...
  get childAthleteCollege => new ExpenseSpec(ExpenseType.COLLEGE_EXPENSE,
      new FlowSpec("Private school", "source", cFlowSequenceSpec()
    ..dateRange = dateRange(date(2022, 1, 1), date(2026, 1, 1))
    ..paymentFrequency = PaymentFrequency.ANNUAL
    ..initialValue = dv(date(2013, 1, 1), 400.0)));

  get palimony => new ExpenseSpec(ExpenseType.ALIMONY, new FlowSpec("Ex-wife-1",
      "source", cFlowSequenceSpec()
