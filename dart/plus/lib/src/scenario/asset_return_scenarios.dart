...
class ReturnGeometricShift implements ScenarioGenerator {
  ReturnGeometricShift(this.shift);

  double shift = 1.0;
  // custom <class ReturnGeometricShift>

  AssumptionModel generateScenario(Dossier _, AssumptionModel assumptionModel) {
    ...
    final result = (new AssumptionModelBuilder.copyFrom(
        assumptionModel)..balanceSheetAssumptions = _visitBalanceSheetCurves(
            assumptionModel.balanceSheetAssumptions,
            (RateCurve curve) => curve * shift)).buildInstance();
...
class ReturnArithmeticShift implements ScenarioGenerator {
...
class ReturnUnfavorableSequence implements ScenarioGenerator {
...
class ReturnFavorableSequence implements ScenarioGenerator {
...
_visitBalanceSheetCurves(BalanceSheetAssumptions assumptions,
    CurveAdjuster curveAdjuster) {
  var builder = new BalanceSheetAssumptionsBuilder.copyFrom(assumptions);

  builder.accountAssumptions = valueApply(
      builder.accountAssumptions,
  ...
  builder.instrumentAssumptions = valueApply(
      builder.instrumentAssumptions,
      (InstrumentAssumptions source) =>
          source.adjustHoldingReturnCurves(curveAdjuster));

  assert(pre != builder.instrumentAssumptions);

  builder.assetAssumptions = valueApply(
      builder.assetAssumptions,
