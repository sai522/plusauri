...
class ForecastGrid {
      ...
      { AssumptionModel assumptionModelOverride, bool trackDetails : false })
  {
    ...
    _completeYearRange = new YearRange(requestedYearRange.start-1,
        requestedYearRange.end);
  ...
  GridYear gridYear(int year) =>
    _gridYears[year-_completeYearRange.start];

  GridHolding gridHolding(GridYear gridYear, HoldingKey holdingKey) =>
    gridYear.gridHoldings[_matchingGridHolding(holdingKey)];

  void visitPrepYear(GridYearVisitor visitor) =>
    visitor(_gridYears[0]);
  ...
  void visitIncomesOnYear(int year, GridFlowVisitor visitor) {
    _incomeInfos.forEach((IncomeInfo incomeInfo) {
      double sum = incomeInfo
        .flowsOnYearIterator(year)
        .fold(0.0,
            (double sum, FlowDetail flowDetail) => flowDetail.flow + sum);
  ...
  void visitObligatoryExpensesOnYear(int year, GridFlowVisitor visitor) {
    _expenseInfos
      .where((ExpenseInfo expenseInfo) => expenseInfo.expenseType.isObligatory)
      .forEach((ExpenseInfo expenseInfo) {
        double sum = expenseInfo
          .flowsOnYearIterator(year)
          .fold(0.0,
              (double sum, FlowDetail flowDetail) => flowDetail.flow + sum);
  ...
  void visitAccountOtherHoldings(GridYear gridYear, String accountName,
    ...
    final accountInfo = _accountInfos.firstWhere((AccountInfo accountInfo) =>
  ...
  int _findAccountStartIndex(String accountName) =>
    _accountInfos.firstWhere((AccountInfo accountInfo) =>
  ...
  int _findHoldingInfoIndex(String accountName, String holdingName) {
    ...
    return startIndex + _holdingInfos
      .skip(startIndex)
      .takeWhile((HoldingInfo holdingInfo) =>
          holdingInfo.holdingName != holdingName)
      .length;
  }

  HoldingPeriodBalance createHoldingPeriodBalance(GridYear gridYear,
    String accountName, String holdingName) =>
    _createHoldingPeriodBalance(gridYear,
  ...
  HoldingPeriodBalance _createHoldingPeriodBalance(GridYear gridYear,
    ...
    final instrumentPartitionMappings =
      ...
      .fromAssumptions(periodBalance.endValue,
...
''';
  ...
  String _printGrid() {
    ...
    for(int i=0; i<numAssets; i++) {
      parts.add('A(${_assetInfos[i].name}) => ${_gridYears.map((gy) => pv(gy.year, gy.assetBalances[i].balance)).join(", ")}');
    }
    for(int i=0; i<numLiabilities; i++) {
      parts.add('L(${_liabilityInfos[i].name}) => ${_gridYears.map((gy) => pv(gy.year, gy.liabilityBalances[i].balance)).join(", ")}');
    }
    for(int i=0; i<numHoldings; i++) {
      parts.add('H(${_holdingInfos[i].holdingKey}) =>\n\t${_gridYears.map((gy) => gridHolding(gy.gridHoldings[i], gy.year)).join(",\n\t")}');
    }
    return parts.join('\n');
  }
  ...
  void _initializeLinkPreferences() {
    ...
    _preferredExpenseSources = valueApply(_dossier.preferredExpenseSources,
          (List<HoldingKey> holdingKeys) =>
        _matchingGridHoldings(holdingKeys).toList());
    _incomeHoldings = _matchingGridHoldings(_dossier.incomeHoldingKeys)
...
''');
  ...
  Iterable<int> _matchingGridHoldings(Iterable<HoldingKey> holdingKeys) =>
    holdingKeys.map((HoldingKey holdingKey) =>
        _matchingGridHolding(holdingKey));


  void _initializeAssumptions(AssumptionModel assumptionModelOverride) {
    _assumptionModel = assumptionModelOverride == null?
      _dossier.assumptionModel : assumptionModelOverride;
  ...
  void _initializeGridYear(GridYear gridYear) {
    ...
    for(int i=0; i<numHoldings; i++) {
      ...
      final reinvestmentPolicy = _balanceSheetAssumptions.
        getReinvestmentPolicy(holdingKey);
  ...
  void _initializeGrid() {
    ...
    for(int i=0; i<numYears; i++) {
      ...
      final gridYear =
        _gridYears[i] = new GridYear(startDate, endDate,
          _holdingInfos.length, _assetInfos.length,
  ...
  IncomeInfo _createIncomeInfo(String incomeName) {
    ...
    incomeSpec.visitFlows(_completeDateRange, (Date date, double flow) {
      ...
      final flowDetail = new FlowDetail(date,
          incomeSpec.incomeType.value, flow,
          incomeName);
  ...
  ExpenseInfo _createExpenseInfo(String expenseName) {
    ...
    expenseSpec.visitFlows(_completeDateRange, (Date date, double flow) {
      final gridYear = this.gridYear(date.year);
      final flowDetail = new FlowDetail(date,
          expenseSpec.expenseType.value, -flow,
          expenseName);
      gridYear.flowDetails.add(flowDetail);
      expenseFlowDetails.add(flowDetail);
    });
    final result =  new ExpenseInfo(expenseName, expenseSpec,
        expenseFlowDetails);
    return result;
  }

  void _createExpenseInfos() {
    _expenseInfos = _flowModel
      .expenseModel
      .keys
  ...
  void _createAccountInfos() {
    ...
    if(_holdingInfos.length > 0) {
      ...
      void createAccount() {
        ...
        _accountInfos.add(
          new AccountInfo(currentAccount, portfolioAccount,
              startIndex, endIndex));
        startIndex = endIndex;
      }
  ...
  HoldingInfo _createHoldingInfo(HoldingKey holdingKey, Holding holding) {
    ...
    final instrumentPartitions = _balanceSheetAssumptions
      .getInstrumentPartitions(holdingKey);

    final distanceToTarget = targetPartitions
      .blendedDistance(instrumentPartitions);

    final curvesAttribution = new CurvesAttribution(
      _balanceSheetAssumptions
      .getHoldingReturns(holdingKey)
      .returns);
  ...
  void _createHoldingInfos() {
    ...
    for(final holdingKey in _dossier.holdingKeys) {
      _holdingInfos[i] = _createHoldingInfo(holdingKey, _dossier.holding(holdingKey));
  ...
  void _growHoldingsToFirstYear() {
    ...
    for(int i=0; i<numHoldings; ++i) {
      ...
      final avgCostAccumulator =
        isCash?
        new AvgCostAccumulator(initialTotalValue, initialTotalValue, initialTotalValue) :
  ...
  void _updateBalance(GridBalance prevBalance, GridBalance current,
    ...
    current.balance = current.prevBalance *
      growth.scaleFromTo(_prevPeriodEnd, _periodEnd);
  }
  ...
  void _pullDistributionsFromAttribution(Attribution attribution,
    ...
    var qd = attribution.contributionDelta(HoldingReturnType.QUALIFIED_DIVIDEND,
        startValue);
    ...
    var interest = attribution.contributionDelta(
      HoldingReturnType.INTEREST, startValue);
...
''');
  ...
  void _forecastNextGridYear(int gridYearIndex) {
    ...
    _growAssetsFromPrevious(_prevGridYear.assetBalances,
        _currentGridYear.assetBalances);
    _growLiabilitiesFromPrevious(_prevGridYear.liabilityBalances,
        _currentGridYear.liabilityBalances);
    _growHoldingsFromPrevious(_prevGridYear.gridHoldings,
  ...
  void _processTaxBill() {
    _currentGridYear.taxBill = _prevGridYear
      ...
      for(final gridHoldingIndex in sources) {
        ...
        if(_trackDetails) {
          if(newFlow != flow) {
            _currentGridYear
              .forecastDetails
              .registerAccountChange(flowDetail, flow - newFlow,
  ...
  void _processTargetedIncomeFlow(FlowDetail flowDetail) {
    ...
    if(_trackDetails) {
      _currentGridYear
        .forecastDetails
        .registerAccountChange(flowDetail, flow, ReserveHolding);
    }
  ...
  void _processFlows() {
    _yearFlows.forEach((FlowDetail flowDetail) =>
        flowDetail.flow < 0.0?
  ...
  void _createAssetInfos() {
    ...
    int i = 0;
    _dossier.visitAssets((String assetName, Asset asset) =>
        _assetInfos[i++] =
        new AssetInfo(assetName, asset,
  ...
  void _createLiabilityInfos() {
    ...
    _dossier.visitLiabilities((String liabilityName, Liability liability) =>
        _liabilityInfos[i++] =
        new LiabilityInfo(liabilityName, liability,
...
class HoldingDistance
  implements Comparable<HoldingDistance> {
...
class TaxBreakdown {
  ...
  double get total =>
    pension + socialSecurity + rental + labor + capitalGain +
    qualifiedDividends + unqualifiedDividends + capitalGainDistributions +
    interest;

  TaxBreakdown operator +(TaxBreakdown other) =>
    new TaxBreakdown(pension + other.pension,
  ...
  TaxBreakdown operator -() =>
    new TaxBreakdown(-pension, -socialSecurity, -rental,
        -labor, -capitalGain, -qualifiedDividends, -unqualifiedDividends,
  ...
  TaxBreakdown generateTaxBill(TaxRateAssumptions taxRateAssumptions, Date startOfPeriod) {
...
class GridYear {
  ...
  GridYear(this.startDate, this.endDate, int numHoldings,
      int numAssets, int numLiabilities) :
  ...
  GridBalance get totalHoldings {
    double prevBalance = 0.0, balance = 0.0;
  ...
  DistributionBreakdown get shelteredDistributions =>
    gridHoldings.fold(new DistributionBreakdown.empty(),
        (DistributionBreakdown prev, GridHolding holding)
        => prev + holding.shelteredDistributions);


  double get shelteredCapitalGains =>
    gridHoldings.fold(0.0,
        (double prev, GridHolding holding)
        => prev + holding.shelteredCapitalGain);

  GridBalance get totalNonHoldingAssets => _sumHoldings(assetBalances);
  GridBalance get totalAssets => totalHoldings
    ..plusEqual(totalNonHoldingAssets);
  ...
  Iterable<FlowDetail> get standardIncomeFlowDetails {
...

    {
      ...
      result.add(new FlowDetail(endDate,
              ...
              unqualified, '^Unqualified Dividends'));
    }
    ...
    {
      final interestDistribution =
      distributionSummary.distributed.interest;
      result.add(new FlowDetail(endDate,
  ...
  _sumHoldings(Iterable<GridBalance> balances) =>
    balances.fold(
      new GridBalance.empty(),
      (GridBalance result, GridBalance elm) {
...
class GridHolding {
  ...
  double netTrade = 0.0;
  DistributionBreakdown distributedDistributions = new DistributionBreakdown.empty();
  DistributionBreakdown reinvestedDistributions = new DistributionBreakdown.empty();
  DistributionBreakdown shelteredDistributions = new DistributionBreakdown.empty();
  AvgCostAccumulator avgCostAccumulator;
  ...
  Map<HoldingReturnType, num> get growthDetails =>
    valueApply(growthAttribution.attribution,
        (num factor) => prevBalance*factor);
  ...
  bool get _invariant =>
    balance >= 0.0 &&
    ((balance + netTrade) >= 0.0);

  toString() =>
    '$holdingKey (prev:${moneyFormat(prevBalance)},'
    'bal:${moneyFormat(balance)},'
    'fullb:${moneyFormat(fullBalance)},'
    'nettr:${moneyFormat(netTrade)},'
...
typedef GridLiabilityVisitor(LiabilityInfo liabilityInfo, GridBalance gridBalance);
typedef GridHoldingVisitor(HoldingInfo holdingInfo, GridHolding gridHolding);
typedef GridHoldingValueVisitor(HoldingInfo holdingInfo, double startValue,  double endValue);
