...
class _Holdings {
  ...
  get ibmDrip => (new Holding(
        HoldingType.STOCK,
        dv(date(2000,1,1), 50.0),
        dv(date(2002,1,1), 50.0),
        50.0*45.0));

  get msftQdrip => (new Holding(
        HoldingType.STOCK,
        dv(date(2000,1,1), 50.0),
        dv(date(2002,1,1), 50.0),
        50.0*45.0));
  ...
  get msft => (new Holding(
        HoldingType.STOCK,
        dv(date(2000,1,1), 50.0),
        dv(date(2002,1,1), 50.0),
        50.0*45.0));

  get nodiv => (new Holding(
        HoldingType.STOCK,
        dv(date(2000,1,1), 50.0),
        dv(date(2002,1,1), 50.0),
        50.0*45.0));
...
final _sampleAccounts = {
  'college junior' : (new PortfolioAccount(
        AccountType.COLLEGE_IRS529,
        'College Fund',
        'johnDoe',
        _holdings.all,
        (new Holding(
  ...
  'etrade' : (new PortfolioAccount(
        AccountType.INVESTMENT,
        'Basic holding account',
        'johnDoe',
        _holdings.all,
        (new Holding(
          HoldingType.STOCK,
          dateValue(date(2000,1,1), 1000.0),
          dateValue(date(2000,1,1), 1.0),
          950.0))))
};
...
class _BalanceSheets {
  ...
  get wealthy => new BalanceSheet(date(2014,1,1),
      _assets.all, _liabilities.all, _sampleAccounts);
