...
// custom <main>
  ...
  var t1 = new Trade(date(2001, 1, 1), 'FOO', BUY, 100.0, 20.0),
      t2 = new Trade(date(2001, 1, 2), 'FOO', SELL, 100.0, 20.0),
      t3 = new Trade(date(2001, 1, 3), 'FOO', BUY, 100.0, 20.0),
      t4 = new Trade(date(2001, 1, 4), 'FOO', BUY, 100.0, 20.0);
  ...
  group('trade journal', () {
    test('keeps trades sorted and date range', () {
      ...
      expect(
          tj.dateRange, dateRange(date(2001, 1, 1), date(2001, 1, 2).nextDay));
      ...
      expect(
          tj.dateRange, dateRange(date(2001, 1, 1), date(2001, 1, 4).nextDay));
    });
  });
  ...
  group('PortfolioHistory', () {
    ...
    test('from portfolio - prior trades', () {
      var p = new Portfolio(date(2002, 1, 1), {'FOO': 5000.0,});
      p.createPortfolioHistory(journal);
    });
  });
  ...
  final journals = {
    new TradeJournal([buy(1.0, 100.5), buy(1.0, 101.5), sell(2.0, 100.0)]): {
      'totalGain': -2.0,
      'shortTermGain': -2.0,
      'longTermGain': 0.0
    },
    new TradeJournal([buy(1.0, 101.5), buy(1.0, 100.5), sell(2.0, 102.0)]): {
      'totalGain': 2.0,
      'shortTermGain': 2.0,
      'longTermGain': 0.0
    },
  ...
  group('cumulative gains', () {
    journals.forEach((journal, expected) {
      ...
      closers.forEach((method, closer) {
        final tta = new TradeTaxAssessor(journal)..closeLots(closer, null);
  ...
  group('AvgCostAccumulator', () {
    test('simple scenario', () {
      ...
      // Buy 1000 @ $10
      avgCostAccumulator.markThenBuy(
          avgCostAccumulator.totalValue * 10.0 / 20.0, 10000.0);
      // Buy 1250 @ $8
      avgCostAccumulator.markThenBuy(
          avgCostAccumulator.totalValue * 8 / 10, 10000.0);
      // Now sell 1000 @ $19
      double gain = avgCostAccumulator.markThenSell(
          avgCostAccumulator.totalValue * 19.0 / 8.0, 19000.0);
      expect(closeEnough(gain, 5666.66666666), true);

      // Extend the example - sell another 1000 @ $22
      gain = avgCostAccumulator.markThenSell(
          avgCostAccumulator.totalValue * 22 / 19.0, 22000.0);
      expect(closeEnough(gain, 8666.666666), true);

      // Buy 1000 @ 10
      avgCostAccumulator.markThenBuy(
          avgCostAccumulator.totalValue * 10.0 / 22.0, 10000.0);
      // Sell 1000 @ 15
      gain = avgCostAccumulator.markThenSell(
          avgCostAccumulator.totalValue * 15.0 / 10.0, 15000.0);
    ...
    test('simple scenario - no gains', () {
      ...
      for (int i = 0; i < 3; i++) {
        expect(closeEnough(avgCostAccumulator.markThenSell(
            avgCostAccumulator.totalValue, 1000.0), 0.0), true);
      }
    });
    ...
    test('simple scenario - 10% gains', () {
      ...
      for (int i = 1; i < 3; i++) {
        avgCostAccumulator.markThenBuy(
            avgCostAccumulator.totalValue * 1.1, 1000.0);
      }

      double totalGain = avgCostAccumulator.markThenSell(
          avgCostAccumulator.totalValue, avgCostAccumulator.totalValue);

      final expectedGain =
          ((((1000.0 * 1.1) + 1000.0) * 1.1 + 1000.0) - 3000.0);
