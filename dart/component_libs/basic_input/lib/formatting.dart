...
var _currencyFormatTwoDecimals =
    new NumberFormat(r'$#,##0.00;$(#,##0.00)', "en");
...
double moneyRoundCent(num amount) => (amount * 100.0).round() / 100.0;

int moneyRoundDollar(num amount) => amount.round();

String moneyFormat(num n, [bool showDecimals]) =>
    ((showDecimals == null && (n.abs() <= 99999.9999)) || showDecimals == true)
...
num roundToCeil(num n, [int digits = 2]) => n < 0
    ? _roundTo(n, _floorRounder, digits)
    : _roundTo(n, _ceilRounder, digits);

num roundToFloor(num n, [int digits = 2]) => n < 0
    ? _roundTo(n, _ceilRounder, digits)
    : _roundTo(n, _floorRounder, digits);
...
String moneyShortForm(num n) {
  final absn = n.abs();
  return (absn < THOUSAND)
      ? moneyFormat(n)
      : (absn < MILLION)
          ? '${_currencyFormatK.format(n/THOUSAND)}'
          : (absn < BILLION)
              ? '${_currencyFormatM.format(n/MILLION)}'
              : (absn < TRILLION)
                  ? '${_currencyFormatB.format(n/BILLION)}'
