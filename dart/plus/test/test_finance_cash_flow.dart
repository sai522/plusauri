...
// custom <main>
  ...
  group('test_finance_cash_flow.dart', () {
    group('cash_flow', () {
      ...
      var rc1 =
          rateCurve([dv(date(1900, 1, 1), 0.03), dv(date(2002, 1, 1), 0.04),]);
      ...
      test('valueOn', () {
        [
          ...
          double expected = moveValueInTime(
                  100.0, 0.03, date(2001, 1, 1), end) +
              moveValueInTime(200.0, 0.03, date(2002, 1, 1), end) +
              (moveValueInTime(moveValueInTime(
                      300.0, 0.04, date(2003, 1, 1), date(2002, 1, 1)), 0.03,
                  date(2002, 1, 1), end)) +
              (moveValueInTime(moveValueInTime(
                      400.0, 0.04, date(2004, 1, 1), date(2002, 1, 1)), 0.03,
                  date(2002, 1, 1), end));

          expect(closeEnough(actual, expected), true);
        });

        [date(2005, 1, 1), date(2006, 1, 1)].forEach((end) {
          double actual = cf1.valueOn(end, rc1);
          double expected = moveValueInTime(moveValueInTime(
                      100.0, 0.03, date(2001, 1, 1), date(2002, 1, 1)), 0.04,
                  date(2002, 1, 1), end) +
