...
class GridTimelineModel implements ITimelineModel {
  GridTimelineModel(this._forecastGrid, this._inflation) {
    _init();
  }
  ...
  _initFlowItems() {
    ...
    for (int year = startYear; year < endYear; year++) {
      ...
      for (final incomeInfo in _forecastGrid.incomeInfos) {
        incomeInfo.flowsOnYearIterator(year).forEach((FlowDetail flowDetail) {
      ...
      for (final expenseInfo in _forecastGrid.expenseInfos) {
        expenseInfo.flowsOnYearIterator(year).forEach((FlowDetail flowDetail) {
          assert(flowDetail.flow < 0.0);
      ...
      netFlows[year - startYear] = new YearIncomeFlows()
        ..year = year
        ..netFlow = netIncome - netExpense;
    }

    _netIncomes = new ForecastFlows('Net Inflows', netIncomeFlows)
      ..findMinMax();
    _netExpenses = new ForecastFlows('Net Outflows', netExpenseFlows)
      ..findMinMax();
  ...
  _initBalanceItems() {
    ...
    double prevNetAssets = 0.0,
        prevNetLiabilities = 0.0,
        prevNetWorth = 0.0,
        prevReserves = 0.0;

    _forecastGrid.visitPrepYear((GridYear gridYear) => _gridTally(gridYear,
        (double netAssets, double netLiabilities, double netWorth,
        double reserves) {
    ...
    _forecastGrid.visitGridYears((GridYear gridYear) {
      ...
      _gridTally(gridYear, (double netAssets, double netLiabilities,
          double netWorth, double reserves) {
        netWorthPbs[i] = new PeriodBalance.courtesy(
            dateValue(startDate, prevNetWorth), dateValue(endDate, netWorth));

        netAssetsPbs[i] = new PeriodBalance.courtesy(
            dateValue(startDate, prevNetAssets), dateValue(endDate, netAssets));
        ...
        reservesPbs[i] = new PeriodBalance.courtesy(
            dateValue(startDate, prevReserves), dateValue(endDate, reserves));
    ...
    _netLiabilities =
        new ForecastBalances('Net Liabilities', netLiabilitiesPbs);
...
class GridBalanceSheetModel implements IBalanceSheetModel {
  GridBalanceSheetModel(this._forecastGrid, this._gridYear) {
    _init();
  }
  ...
  void visitAccounts(BSMAccountHoldingVisitor visitor) => _forecastGrid
      .visitAccounts((AccountInfo accountInfo) =>
  ...
  PeriodBalance get totalHoldings => _fromGridBalance(_gridYear.totalHoldings);

  PeriodBalance get totalNonHoldingAssets =>
      _fromGridBalance(_gridYear.totalNonHoldingAssets);

  PeriodBalance get totalAssets => _fromGridBalance(_gridYear.totalAssets);
  ...
  PeriodBalance get reserves => new PeriodBalance()
    ..start = new DateValue(_gridYear.startDate, _gridYear.reserves.prevBalance)
    ..end = new DateValue(_gridYear.endDate, _gridYear.reserves.balance);

  PeriodBalance _fromGridBalance(GridBalance gridBalance) =>
      new PeriodBalance.courtesy(dateValue(startDate, gridBalance.prevBalance),
          dateValue(endDate, gridBalance.balance));

  HoldingPeriodBalance createHoldingPeriodBalance(
      String accountName, String holdingName) => _forecastGrid
          .createHoldingPeriodBalance(_gridYear, accountName, holdingName);
...
class GridIncomeStatementModel implements IIncomeStatementModel {
...
class GridLiquidationSummaryModel implements ILiquidationSummaryModel {
  GridLiquidationSummaryModel(this._forecastGrid, this._gridYear) {
    _init();
  }
  ...
  _init() {
    _gridYear.forecastDetails.flowAccountLinks.forEach(
        (FlowAccountLink flowAccountLink) {
  ...
  void visitCredits(CreditVisitor creditVisitor) {
    ...
    flowAccountLinks
      ...
      creditVisitor(holdingKey, flowDetail == null
          ? NetFlowsLabel
          : flowDetail.name, flowAccountLink.flowAllocation, remainingBalance);
    });
  }
  ...
  void visitDebits(DebitVisitor debitVisitor) {
    ...
    flowAccountLinks
      ...
      final remainingBalance = holdingKey == ReserveHolding
          ? (_originalNetTrade += flowAllocation)
          : (_originalBalances[holdingKey] += flowAllocation);
      debitVisitor(holdingKey, flowDetail == null
          ? NetFlowsLabel
          : flowDetail.name, flowAllocation, remainingBalance);
    });
  }
...
class GridTaxSummaryModel implements ITaxSummaryModel {
  ...
  void visitPension(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.pension,
      _gridYear.taxBill.pension);

  void visitSocialSecurity(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.socialSecurity,
      _gridYear.taxBill.socialSecurity);

  void visitRental(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.rental,
      _gridYear.taxBill.rental);

  void visitLabor(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.labor, _gridYear.taxBill.labor);

  void visitCapitalGain(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.capitalGain,
      _gridYear.taxBill.capitalGain);

  void visitQualifiedDividends(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.qualifiedDividends,
      _gridYear.taxBill.qualifiedDividends);

  void visitUnqualifiedDividends(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.unqualifiedDividends,
      _gridYear.taxBill.unqualifiedDividends);

  void visitCapitalGainDistributions(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.capitalGainDistributions,
      _gridYear.taxBill.capitalGainDistributions);

  void visitInterest(TaxSummaryVisitor visitor) => visitor(
      _previousGridYear.taxableIncomeBreakdown.interest,
...
class GridAnnualForecastModel implements IAnnualForecastModel {
  GridAnnualForecastModel(this._forecastGrid, this._title, this._year) {
    _init();
  }
  ...
  _init() {
    ...
    _liquidationSummaryModel =
        new GridLiquidationSummaryModel(_forecastGrid, gridYear);
  }

  // TODO: clean this up
  CurrentDollarsToggler _currentDollarsToggler = new CurrentDollarsToggler(
      today, new RateCurve([dateValue(date(1900, 1, 1), 0.03)]));
  ...
  void visitExtendedLiabilities(RateCurve discount, AFVisitor visitor) {
    _balanceSheetModel.visitLiabilities(
        (String liabilityName, double start, double end) {
    ...
    thisYear.forEach((String liability, double expense) {
      ...
      visitor(liability,
          lastYearValue == null ? 0.0 : -lastYearValue, -expense, true);
    });
  }
